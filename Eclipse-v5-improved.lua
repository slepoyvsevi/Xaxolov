...
function ESPModule:CreateData()
    local d = {
        ...
        fullBoxInner = newSquare(false),  -- цветной прямоугольник внутри чёрного
        boxFill      = newSquare(true),
        corners      = {},
        healthBg     = newSquare(true),
        healthFill   = newSquare(true),
        healthText   = newText(),         -- HP число при низком здоровье
        nameText     = newText(),
        distText     = newText(),
        weaponText   = newText(),
        tracer       = newLine(),
        bones        = {},
        highlight    = nil,
        btGhost      = nil, btHighlight = nil, btGhostTime = 0,
        btHistory    = {}, btHead = 1, btTail = 0, btLastRecord = 0,
        cachedWeapon = nil, cachedWeaponTick = 0,
        lastChamsType  = nil, lastChamsDepth = nil,
        lastChamsOC    = nil, lastChamsFC    = nil,
        lastChamsFT    = nil, lastChamsOT    = nil,
    }
...
function ESPModule:DestroyData(plr)
    ...
    rem(d.fullBox); rem(d.fullBoxInner); rem(d.boxFill)
    for _, l in ipairs(d.corners) do rem(l) end
    rem(d.healthBg); rem(d.healthFill); rem(d.healthText)
...
function ESPModule:UpdateBox(d, bb)
    ...
    if self.BOX_TYPE == "Full" then
        for i = 1, 8 do local l = d.corners[i]; if l then l.Visible = false end end
        if d.fullBox then
            -- Outline чуть шире для красивого контура
            d.fullBox.Position  = V2new(x - 1, y - 1)
            d.fullBox.Size      = V2new(w + 2, h + 2)
            d.fullBox.Color     = C3rgb(0,0,0)
            d.fullBox.Thickness = 1
            d.fullBox.Visible   = true
        end
        -- Внутренний цветной прямоугольник
        if d.fullBoxInner then
            d.fullBoxInner.Position  = V2new(x, y)
            d.fullBoxInner.Size      = V2new(w, h)
            d.fullBoxInner.Color     = col
            d.fullBoxInner.Thickness = 1
            d.fullBoxInner.Visible   = true
        end
    else
        if d.fullBox      then d.fullBox.Visible      = false end
        if d.fullBoxInner then d.fullBoxInner.Visible = false end
        -- Corner box: длина угла 26% ширины
        local cl  = mathMax(w * 0.26, 6)
        local xw  = x + w
        local yh  = y + h
        local blk = C3rgb(0,0,0)
        local t   = 2.5  -- толщина
        local c   = d.corners
        -- Чёрная тень +1px для читаемости
        local shadow = {
            {V2new(x-1,y-1),   V2new(x+cl+1,y-1)},
            {V2new(x-1,y-1),   V2new(x-1,y+cl+1)},
            {V2new(xw+1,y-1),  V2new(xw-cl-1,y-1)},
            {V2new(xw+1,y-1),  V2new(xw+1,y+cl+1)},
            {V2new(x-1,yh+1),  V2new(x+cl+1,yh+1)},
            {V2new(x-1,yh+1),  V2new(x-1,yh-cl-1)},
            {V2new(xw+1,yh+1), V2new(xw-cl-1,yh+1)},
            {V2new(xw+1,yh+1), V2new(xw+1,yh-cl-1)},
        }
        local coords = {
            {V2new(x,y),   V2new(x+cl,y)},
            {V2new(x,y),   V2new(x,y+cl)},
            {V2new(xw,y),  V2new(xw-cl,y)},
            {V2new(xw,y),  V2new(xw,y+cl)},
            {V2new(x,yh),  V2new(x+cl,yh)},
            {V2new(x,yh),  V2new(x,yh-cl)},
            {V2new(xw,yh), V2new(xw-cl,yh)},
            {V2new(xw,yh), V2new(xw,yh-cl)},
        }
        for i = 1, 8 do
            local l = c[i]; if not l then continue end
            local co = coords[i]
            l.From = co[1]; l.To = co[2]
            l.Color = col; l.Thickness = t; l.Visible = true
        end
    end
...
function ESPModule:UpdateHealth(d, bb, hum)
    ...
    local hp    = hum.Health
    local maxHp = mathMax(hum.MaxHealth, 1)
    local ratio = mathClamp(hp / maxHp, 0, 1)
    local barW  = 5
    local pad   = 5
    local x     = bb.x - barW - pad
    local y     = bb.y
    local h     = bb.h
    ...
    if d.healthBg then
        ...
        d.healthBg.Color    = C3rgb(10, 10, 10)
    ...
    if d.healthFill then
        local fh = mathMax(h * ratio, 2)
        d.healthFill.Position = V2new(x, y + h - fh)
        d.healthFill.Size     = V2new(barW, fh)
        d.healthFill.Filled   = true
        d.healthFill.Color    = self.HEALTH_GRADIENT
            and self:GetHealthColor(ratio)
            or  self.HEALTH_HIGH_COLOR
        d.healthFill.Visible  = true
    end

    -- Текст HP над баром когда ниже 80%
    if d.healthText then
        if ratio < 0.8 then
            d.healthText.Text     = tostring(mathFloor(hp))
            d.healthText.Size     = 10
            d.healthText.Color    = self:GetHealthColor(ratio)
            d.healthText.Outline  = true
            d.healthText.OutlineColor = C3rgb(0,0,0)
            d.healthText.Position = V2new(x + barW * 0.5, y - 12)
...
function ESPModule:UpdateInfo(d, bb, targetPlayer, distance, char)
    ...
    local hpPct  = hum and mathClamp(hum.Health / mathMax(hum.MaxHealth, 1), 0, 1) or 1
    local cx     = bb.cx
    local yAbove = bb.y - 2
    ...
    if self.NAME_ENABLED and d.nameText then
        local hpStr = hum and string.format(" [%d%%]", mathFloor(hpPct * 100)) or ""
        d.nameText.Text         = targetPlayer.DisplayName .. hpStr
        d.nameText.Color        = self.NAME_COLOR
        d.nameText.Size         = 14
        d.nameText.Outline      = true
        d.nameText.OutlineColor = C3rgb(0, 0, 0)
        d.nameText.Center       = true
        yAbove = yAbove - 16
        d.nameText.Position = V2new(cx, yAbove)
    ...
    if self.DISTANCE_ENABLED and d.distText then
        ...
        d.distText.OutlineColor = C3rgb(0, 0, 0)
        d.distText.Center       = true
        d.distText.Position     = V2new(cx, yBelow)
    ...
    if self.WEAPON_ENABLED and d.weaponText then
        ...
        if wep then
            ...
            d.weaponText.OutlineColor = C3rgb(0, 0, 0)
            d.weaponText.Center       = true
            d.weaponText.Position     = V2new(cx, yBelow)
...
function VisualsModule:CreateTracer(startPos, endPos)
	...
	local w = math.clamp(self.TRACER_WIDTH, 0.02, 0.02)  -- Trail.Width в studs, 0.02 = тонкая линия

	tracerObj.trail.Color          = ColorSequence.new({
		ColorSequenceKeypoint.new(0,   self.TRACER_COLOR),
		ColorSequenceKeypoint.new(0.4, self.TRACER_COLOR),
		ColorSequenceKeypoint.new(1,   Color3.new(1, 1, 1)),
	})
	tracerObj.trail.Lifetime       = self.TRACER_LIFETIME
	-- Width = 0.02 studs (физическая), WidthScale масштабирует её визуально
	tracerObj.trail.Width          = 0.02
	tracerObj.trail.WidthScale     = NumberSequence.new({
		NumberSequenceKeypoint.new(0,   math.clamp(self.TRACER_WIDTH * 8, 0.1, 10)),
		NumberSequenceKeypoint.new(0.6, math.clamp(self.TRACER_WIDTH * 5, 0.05, 6)),
		NumberSequenceKeypoint.new(1,   0),
	})
	tracerObj.trail.Transparency   = NumberSequence.new({
		NumberSequenceKeypoint.new(0,   0.05),
		NumberSequenceKeypoint.new(0.5, 0.3),
		NumberSequenceKeypoint.new(1,   1),
	})
	tracerObj.trail.LightEmission  = 1
	tracerObj.trail.LightInfluence = 0
	tracerObj.trail.Brightness     = 1
...
function VisualsModule:AnalyzeGame()
	...
	local p = {
		...
		mapDarkness    = false,
		hasNeon        = false,
		neonDensity    = 0,     -- 0..1 доля Neon-частей
		dominantColor  = Color3.fromRGB(255, 65, 65),
		hasFog         = false,
		hasAtmosphere  = false,
		skyLuminance   = 0.5,   -- яркость неба
		gameStyle      = "hvh", -- hvh / sniper / pistol / smg / shotgun
		mapMood        = "neutral",
		bulletSpeed    = 2048,  -- оценённая скорость пули
		fireRate       = 0.05,  -- оценённый fire rate
		-- Итоговые настройки
		tracerStyle    = "Pulse",
	...
	pcall(function()
		...
		p.mapBrightness = (amb.R + amb.G + amb.B) / 3 + (lighting.Brightness or 1)
		p.mapDarkness   = p.mapBrightness < 0.45

		-- Туман
		p.hasFog = (lighting.FogEnd or 100000) < 2000

		-- Атмосфера
		for _, c in ipairs(lighting:GetChildren()) do
			if c:IsA("Atmosphere") then p.hasAtmosphere = true; break end
		end

		-- Небо
		for _, c in ipairs(lighting:GetChildren()) do
			if c:IsA("Sky") then
				-- Оцениваем яркость по наличию кастомного неба
				p.skyLuminance = 0.6; break
			end
		end

		-- ── Сканирование частей карты (до 600) ────────────────────
		local neon, total, rS, gS, bS = 0, 0, 0, 0, 0
		local colorBuckets = {}  -- группировка цветов

		for _, obj in ipairs(ws:GetDescendants()) do
			if total >= 600 then break end
			if not obj:IsA("BasePart") then continue end
			total = total + 1
			if obj.Material == Enum.Material.Neon then
				neon = neon + 1
				local c = obj.Color
				rS, gS, bS = rS + c.R, gS + c.G, bS + c.B
				-- Бакетируем цвета по 0.2 шагу
				local key = math.floor(c.R*5).."_"..math.floor(c.G*5).."_"..math.floor(c.B*5)
				colorBuckets[key] = (colorBuckets[key] or 0) + 1
			end
		end

		p.neonDensity = total > 0 and (neon / total) or 0
		p.hasNeon     = neon > 3

		-- Доминирующий цвет неона
		if neon > 0 then
			-- Берём самый частый бакет
			local bestKey, bestCount = nil, 0
			for k, cnt in pairs(colorBuckets) do
				if cnt > bestCount then bestCount = cnt; bestKey = k end
			end
			if bestKey then
				local r, g, b = bestKey:match("(%d+)_(%d+)_(%d+)")
				if r then
					p.dominantColor = Color3.new(
						math.clamp(tonumber(r)/5, 0, 1),
						math.clamp(tonumber(g)/5, 0, 1),
						math.clamp(tonumber(b)/5, 0, 1))
				end
			else
				p.dominantColor = Color3.new(rS/neon, gS/neon, bS/neon)
			end
		end

		-- ── Анализ оружий в игре ──────────────────────────────────
		local gunKeywords = {
			sniper  = {"ssg","awp","sniper","scout","rifle","kar98","bolt"},
			pistol  = {"pistol","glock","deagle","p250","revolver","usp"},
			smg     = {"smg","mp5","uzi","mac10","tmp","p90"},
			shotgun = {"shotgun","sg","pump","nova","negev"},
		}

		local found = {}
		local function scanName(name)
			local n = name:lower()
			for style, words in pairs(gunKeywords) do
				for _, w in ipairs(words) do
					if n:find(w, 1, true) then
						found[style] = (found[style] or 0) + 1
					end
				end
			end
		end
		...
		-- Сканируем игроков (Tool в backpack/character)
		for _, plr in ipairs(players:GetPlayers()) do
			local bp = plr:FindFirstChildOfClass("Backpack")
			if bp then for _, t in ipairs(bp:GetChildren()) do if t:IsA("Tool") then scanName(t.Name) end end end
			local ch = plr.Character
			if ch then for _, t in ipairs(ch:GetChildren()) do if t:IsA("Tool") then scanName(t.Name) end end end
		end

		-- Определяем стиль по максимуму
		local maxStyle, maxCount = "hvh", 0
		for style, count in pairs(found) do
			if count > maxCount then maxCount = count; maxStyle = style end
		end
		p.gameStyle = maxStyle

		-- ── Оцениваем скорость пули и fire rate из Configuration ──
		for _, obj in ipairs(ws:GetDescendants()) do
			if obj.Name == "Configuration" and obj:IsA("Configuration") then
				local fr = obj:FindFirstChild("Firerate")
				local bs = obj:FindFirstChild("BulletSpeed") or obj:FindFirstChild("Speed")
				if fr and fr:IsA("NumberValue") then
					p.fireRate = math.max(p.fireRate, fr.Value)
				end
				if bs and bs:IsA("NumberValue") then
					p.bulletSpeed = math.max(p.bulletSpeed, bs.Value)
				end
				break
			end
		end
		...
		-- ── Настроение карты ──────────────────────────────────────
		if p.hasNeon and p.neonDensity > 0.08 then
			p.mapMood = "neon"
		elseif p.mapDarkness or p.hasFog then
			p.mapMood = "dark"
		elseif p.mapBrightness > 1.5 or p.hasAtmosphere then
			p.mapMood = "outdoor"
		else
			p.mapMood = "neutral"
		end
	...
	-- ── Выбор трассера ─────────────────────────────────────────────
	-- Логика: mood × gameStyle → style + color + lifetime + width
	local mood  = p.mapMood
	local style = p.gameStyle

	if mood == "neon" then
		-- Неоновая карта: молния цвета доминирующего неона
		p.tracerStyle    = "Lightning"
		p.tracerColor    = p.dominantColor
		p.tracerLifetime = 0.30
		p.tracerWidth    = 0.35
	elseif mood == "dark" then
		if style == "sniper" then
			-- Тёмная + снайпер: тонкий красный Razor
			p.tracerStyle    = "Razor"
			p.tracerColor    = Color3.fromRGB(220, 30, 30)
			p.tracerLifetime = 0.50
			p.tracerWidth    = 0.20
		else
			-- Тёмная + авто: спираль пурпурного
			p.tracerStyle    = "Helix"
			p.tracerColor    = Color3.fromRGB(160, 20, 220)
			p.tracerLifetime = 0.55
			p.tracerWidth    = 0.45
		end
	elseif style == "sniper" then
		-- Снайпер на любой карте: Razor золотой
		p.tracerStyle    = "Razor"
		p.tracerColor    = Color3.fromRGB(255, 210, 60)
		p.tracerLifetime = 0.55
		p.tracerWidth    = 0.18
	elseif style == "shotgun" then
		-- Дробовик: широкий Pulse
		p.tracerStyle    = "Pulse"
		p.tracerColor    = Color3.fromRGB(255, 140, 20)
		p.tracerLifetime = 0.35
		p.tracerWidth    = 0.70
	elseif style == "smg" then
		-- SMG: быстрый Comet голубой
		p.tracerStyle    = "Comet"
		p.tracerColor    = Color3.fromRGB(80, 200, 255)
		p.tracerLifetime = 0.30
		p.tracerWidth    = 0.28
	elseif mood == "outdoor" then
		-- Открытая карта: Comet небесный
		p.tracerStyle    = "Comet"
		p.tracerColor    = Color3.fromRGB(120, 220, 255)
		p.tracerLifetime = 0.40
		p.tracerWidth    = 0.32
	else
		-- Дефолт HvH: классический Pulse красный
		p.tracerStyle    = "Pulse"
		p.tracerColor    = Color3.fromRGB(255, 55, 55)
		p.tracerLifetime = 0.45
		p.tracerWidth    = 0.40
	end

	-- ── Выбор хит-саунда ─────────────────────────────────────────
	if style == "sniper" then
		p.hitSound = "headshot"       -- резкий звук попадания
	elseif mood == "neon" then
		p.hitSound = "orb"            -- электрический
	elseif mood == "dark" then
		p.hitSound = "tank1"          -- тяжёлый
	elseif style == "shotgun" then
		p.hitSound = "balltap"        -- глухой удар
	elseif style == "smg" then
		p.hitSound = "correct"        -- частый клик
	else
		p.hitSound = "bell"           -- универсальный
	end

	-- ── Выбор Kill-эффекта ────────────────────────────────────────
	if mood == "neon" then
		p.killFX      = "Plasma"
		p.killFXColor = p.dominantColor
	elseif mood == "dark" and style == "sniper" then
		p.killFX      = "Shatter"
		p.killFXColor = Color3.fromRGB(200, 30, 30)
	elseif mood == "dark" then
		p.killFX      = "Vortex"
		p.killFXColor = Color3.fromRGB(100, 0, 160)
	elseif style == "sniper" then
		p.killFX      = "Shatter"
		p.killFXColor = Color3.fromRGB(255, 210, 60)
	elseif style == "shotgun" then
		p.killFX      = "Inferno"
		p.killFXColor = Color3.fromRGB(255, 120, 20)
	elseif style == "smg" then
		p.killFX      = "Cascade"
		p.killFXColor = Color3.fromRGB(80, 200, 255)
	elseif mood == "outdoor" then
		p.killFX      = "Ring"
		p.killFXColor = Color3.fromRGB(120, 220, 255)
	else
		p.killFX      = "Nova"
		p.killFXColor = Color3.fromRGB(255, 55, 55)
	end
...
function VisualsModule:ApplyAutoDetect()
	...
	task.spawn(function()
		...
		-- Хит-саунд
		self.HITSOUND_PRESET = p.hitSound

		-- Килл-эффект (новые ключи: killFX / killFXColor)
		self.KILLFX_TYPE  = p.killFX
		self.KILLFX_COLOR = p.killFXColor

		-- Хитмаркер цвет = цвет трассера
		self._hitMarkerColor = p.tracerColor

		pcall(function()
			local moodMap = { neon="неон", dark="тёмная", outdoor="открытая", neutral="нейтральная" }
			local stylMap = { sniper="снайпер", pistol="пистолет", smg="смг", shotgun="дробовик", hvh="HvH" }
			if self.Notification then
				self.Notification(
					"Eclipse · Auto-Detect",
					(moodMap[p.mapMood] or p.mapMood) .. " · " .. (stylMap[p.gameStyle] or p.gameStyle)
					.. "\n↪ " .. p.tracerStyle .. " / " .. p.hitSound .. " / " .. p.killFX
				)
			end
		end)
	end)
...
function AimbotModule.new(cfg)
    ...
    -- Настройки
    self.ENABLED       = false
    self.MODE          = "Camera"   -- Camera / Body / Silent / Velocity / Combined
    self.FOV           = 120
    self.SMOOTH        = 5          -- 1=медленно .. 10=моментально
    ...
    self._origMaxZoom  = nil
    self._origMinZoom  = nil
...
function AimbotModule:IsTeammate(p)
    ...
    -- Оба должны быть в одной команде — если хоть у одного нет команды, не тиммейт
    if not me.Team or not p.Team then return false end
    return me.Team == p.Team
end

function AimbotModule:HasWall(origin, targetChar, targetPos)
    if not self.WALL_CHECK then return false end
    local dir = targetPos - origin
    if dir.Magnitude < 0.5 then return false end
    local params = RaycastParams.new()
    params.FilterType = Enum.RaycastFilterType.Exclude
    -- Исключаем СЕБЯ и ЦЕЛЬ — иначе луч бьётся о цель и думает что стена
    local myChar = self.player.Character
    local excl = {}
    if myChar then table.insert(excl, myChar) end
    if targetChar then table.insert(excl, targetChar) end
    params.FilterDescendantsInstances = excl
    local res = self.Workspace:Raycast(origin, dir.Unit * dir.Magnitude, params)
    return res ~= nil  -- если что-то ударилось между нами и целью — стена
end
...
function AimbotModule:FindTarget()
    ...
    for _, p in ipairs(self.Services.Players:GetPlayers()) do
        ...
        if d2 >= fov2 then continue end
        if d2 >= bestD then continue end

        -- Wall check
        if self:HasWall(origin, char, part.Position) then continue end
...
function AimbotModule:PredictPos(char)
    ...
    local dt   = ping * 0.5 + 0.05
    return part.Position + Vector3.new(vel.X, 0, vel.Z) * dt
end

-- ── Smooth alpha (1=медленно, 10=моментально) ────────────────────
local function _alpha(smooth)
    -- нелинейная: 1→0.012, 5→0.10, 10→0.55
    local s = math.clamp(smooth, 1, 10) / 10
    return s * s * 0.55
end

-- ── Aim modes ────────────────────────────────────────────────────
-- Camera: плавно поворачивает камеру к цели (самый незаметный)
function AimbotModule:AimCamera(tp)
    local cam    = self.Workspace.CurrentCamera
    local origin = cam.CFrame.Position
    local dir    = tp - origin
    if dir.Magnitude < 0.5 then return end
    local goal = CFrame.lookAt(origin, origin + dir.Unit)
    cam.CFrame = cam.CFrame:Lerp(goal, _alpha(self.SMOOTH))
end

-- Body: поворачивает HRP (тело) — агрессивнее
function AimbotModule:AimBody(tp)
    ...
    hrp.CFrame = hrp.CFrame:Lerp(goal, _alpha(self.SMOOTH))
end

-- Silent: не двигает игрока, только переопределяет куда летит пуля
...
-- Velocity: Camera + дополнительное упреждение по скорости цели
function AimbotModule:AimVelocity(target, tp)
    local char = target.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then self:AimCamera(tp); return end
    local vel   = root.AssemblyLinearVelocity
    local ping  = self.Services.Players.LocalPlayer:GetNetworkPing()
    local extra = Vector3.new(vel.X, vel.Y * 0.3, vel.Z) * ping
    self:AimCamera(tp + extra)
end

-- Combined: Camera + Body вместе
    ...
    task.spawn(function()
        for i = 1, 20 do
            ...
            lp.CameraMinZoomDistance = math.min(
                lp.CameraMinZoomDistance + self.TP_DISTANCE / 20,
                self.TP_DISTANCE)
            task.wait(0.02)
        end
...
function AimbotModule:UpdateTP()
    ...
    local head = char:FindFirstChild("Head")
    if not head then return end
    if (cam.CFrame.Position - head.Position).Magnitude < 2.5 and not self._tpActive then
...
function AimbotModule:_setupFov()
    -- Всегда используем Drawing API если есть (быстрее и точнее)
    if Drawing then
        ...
        self._fovCircle = D("Circle", {
            Visible   = false, Filled = false,
            NumSides  = 80,    Thickness = self.FOV_THICKNESS,
            Color     = self.FOV_COLOR, Radius = self.FOV,
        })
        self._fovPulse = D("Circle", {
            Visible   = false, Filled = false,
            NumSides  = 60,    Thickness = 0.7,
            Color     = self.FOV_COLOR, Radius = self.FOV * 0.9,
        })
        self._crossH = D("Line", {
            Visible   = false, Thickness = 1.2,
            Color     = Color3.new(1,1,1), Transparency = 0.55,
        })
        self._crossV = D("Line", {
            Visible   = false, Thickness = 1.2,
            Color     = Color3.new(1,1,1), Transparency = 0.55,
        })
        self._distLabel = D("Text", {
            Visible  = false, Size = 13, Outline = true,
            Color    = self.FOV_COLOR,
            OutlineColor = Color3.new(0,0,0),
        })
        self._nameLabel = D("Text", {
            Visible  = false, Size = 12, Outline = true,
    ...
    img.ImageColor3 = self.FOV_COLOR; img.ImageTransparency = 0.3
    img.AnchorPoint = Vector2.new(0.5,0.5)
...
function AimbotModule:_updateFov(target)
    if not (self.SHOW_FOV and self.ENABLED) then
        if self._fovCircle then
            self._fovCircle.Visible = false
            if self._fovPulse  then self._fovPulse.Visible  = false end
            if self._crossH    then self._crossH.Visible    = false end
            if self._crossV    then self._crossV.Visible    = false end
            if self._distLabel then self._distLabel.Visible = false end
            if self._nameLabel then self._nameLabel.Visible = false end
        elseif self._fovGui then
            self._fovGui.Enabled = false
        end
        return
    end
    ...
    if self._fovCircle then
        local pulseDelta = math.sin(t * 8) * r * 0.025
        local col = self.FOV_COLOR

        self._fovCircle.Position  = Vector2.new(cx, cy)
        self._fovCircle.Radius    = r
        self._fovCircle.Color     = col
        self._fovCircle.Thickness = self.FOV_THICKNESS
        self._fovCircle.Visible   = true

        -- Пульс-круг: красный при наводке, цвет FOV без цели
        self._fovPulse.Position    = Vector2.new(cx, cy)
        self._fovPulse.Radius      = r * 0.88 + pulseDelta
        self._fovPulse.Color       = hasTarget and Color3.fromRGB(255,50,50) or col
        self._fovPulse.Transparency = hasTarget and 0.35 or 0.7
        self._fovPulse.Visible     = true

        -- Crosshair
        local csz = hasTarget and 7 or 5
        local ccol = hasTarget and Color3.fromRGB(255,50,50) or Color3.new(1,1,1)
        self._crossH.From    = Vector2.new(cx - csz, cy)
        self._crossH.To      = Vector2.new(cx + csz, cy)
        self._crossH.Color   = ccol; self._crossH.Visible = true
        self._crossV.From    = Vector2.new(cx, cy - csz)
        self._crossV.To      = Vector2.new(cx, cy + csz)
        self._crossV.Color   = ccol; self._crossV.Visible = true

        -- Дистанция + имя при наводке
        if hasTarget and target.Character then
            ...
            if root and myRoot then
                ...
                self._distLabel.Position = Vector2.new(cx + r + 8, cy - 6)
                self._distLabel.Color    = self.FOV_COLOR
                self._distLabel.Visible  = true
                self._nameLabel.Text     = target.Name
                self._nameLabel.Position = Vector2.new(cx + r + 8, cy + 8)
                self._nameLabel.Visible  = true
            end
        else
    ...
    elseif self._fovFrame then
        self._fovFrame.Size = UDim2.new(0, r*2, 0, r*2)
        self._fovFrame.ImageColor3 = self.FOV_COLOR
        self._fovGui.Enabled = true
    end
end
...
function AimbotModule:IsActive()
    ...
    if self.HOLD_KEY ~= Enum.KeyCode.Unknown then
        return self.UIS:IsKeyDown(self.HOLD_KEY)
    end
    return true
end
...
function AimbotModule:Start()
    ...
    self._conn = self.RunService.Heartbeat:Connect(function()
        ...
        local target = active and self:FindTarget() or nil
        ...
        if     mode == "Camera"   then self:AimCamera(tp)
        elseif mode == "Body"     then self:AimBody(tp)
...
function AimbotModule:Stop()
    ...
    for _, k in ipairs({"_fovCircle","_fovPulse","_crossH","_crossV","_distLabel","_nameLabel"}) do
...
function AimbotModule:SetTeamCheck(v)    self.TEAM_CHECK    = v end
...
local _ConfigMgr = ConfigManagerClass.new(nil, VisualsInstance, ESPInstance, WorldInstance)

VisualsInstance._resolverOnHit  = nil
VisualsInstance._resolverOnMiss = nil
...
ABMain:Dropdown({ Name = "Mode",           Flag = "ABMode",       Default = {"Camera"},
    Items = {"Camera","Body","Silent","Velocity","Combined"}, Multi = false,
...
ABMain:Slider({ Name = "FOV Size",         Flag = "ABFOV",        Min=20,Max=500,Default=120,Suffix=" px",
...
ABMain:Toggle({ Name = "Team Check",       Flag = "ABTeam",       Default = true,
    Callback = function(v) AimbotInstance:SetTeamCheck(v) end })
...
ABAdv:Button({ Name = "Back to 1st Person",Callback = function() AimbotInstance:DisableThirdPerson() end })
